...
# $$
#
# > **Note**: we've introduced `integrated_error` and set it to 0.0 at
# initialization.  We also now need to know how much time `dt` has elapsed
# between successive calls to `thrust_control` as this influences the
# integrated error.
...
# #### TODO 2 - Adjust gains
# Run the simulation code below with `K_I = 0.0` to remind yourself what a PD
# controller does when there's a large mass_error (remember, a PID controller
# with zero integral gain is just a PD controller).
#
# Then slowly increase `K_I` and rerun the cell.  Note what happens to the
# error.
...
K_P = 20.0
K_D = 10.0
K_I = 3.5 # TODO - increase to 0.5, 1, 2, etc...
AMPLITUDE = 0.5
PERIOD = 0.4
